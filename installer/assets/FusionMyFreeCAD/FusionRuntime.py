...
def _addon_directory():
    # FreeCAD executes InitGui.py directly and does not always define __file__.
    source_path = globals().get("__file__")
    if source_path:
        return os.path.dirname(os.path.abspath(source_path))
    return os.path.join(App.getUserAppDataDir(), "Mod", "FusionMyFreeCAD")


ADDON_DIR = _addon_directory()
...
def apply_preferences():
    ...
    # Keep FreeCAD's native Model/Tasks browser visible by default.
