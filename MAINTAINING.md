...
4. The bundled SearchBar and FreeCAD-Ribbon snapshots are loaded from `vendor/`.
...
| Change the vendored Ribbon or search engine | `vendor/` | Treat as a controlled third-party update; preserve licenses and local patches |
...
`vendor/FreeCAD-Ribbon` and `vendor/SearchBar` are runtime snapshots, not ordinary project source.
...
Avoid broad formatting changes in `vendor/`; they make future upstream comparisons much harder.
...
5. Record what was tested and what was not tested.
