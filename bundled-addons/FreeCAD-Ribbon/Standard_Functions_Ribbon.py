...
def ColorConvertor(ColorRGB: [], Alpha: float = 1, Hex=False, KeepHexAlpha=True):
    ...
    result = (ColorRed, colorGreen, colorBlue, Alpha)
    if Hex is True:
        channels = [ColorRed, colorGreen, colorBlue]
        if KeepHexAlpha:
            channels.append(Alpha)
        result = "#" + "".join("{:02x}".format(max(0, min(255, round(value * 255)))) for value in channels)
