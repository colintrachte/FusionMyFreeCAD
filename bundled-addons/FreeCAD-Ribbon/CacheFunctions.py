...
def DownLoadIcons():
    ...
    with open(os.path.join(pathIcons, "3dx_pivot.png"), "wb") as file:
        file.write(file_content)

        
def CheckDataFileVersion():
    """Ensure the cache exists without putting a modal dialog behind other startup UI."""
    DataFile = os.path.join(ConfigDirectory, "RibbonDataFile.dat")
    language = App.ParamGet("User parameter:BaseApp/Preferences/General").GetString("Language")

    def regenerate(reason):
        App.Console.PrintMessage("FreeCAD Ribbon: {} Generating command cache...\n".format(reason))
        try:
            return bool(CreateCache())
        except Exception as error:
            App.Console.PrintWarning("FreeCAD Ribbon cache generation failed: {}\n".format(error))
            return False

    if not os.path.isfile(DataFile):
        if not regenerate("First startup."):
            return False
    ...
    try:
        ...
        if not regenerate("The existing cache is unreadable."):
            return False
        with open(DataFile, "r", encoding="utf-8") as file:
            data = json.load(file)

    commands = data.get("List_Commands", [])
    stale = (
        data.get("dataVersion") != DataFileVersion
        or data.get("Language", "") != language
        or any(len(item) < 5 for item in commands)
    )
    if stale:
        return regenerate("The cache is out of date.")
    return True
