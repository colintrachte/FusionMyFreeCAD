...
def _sketcher_panel(layout, name):
    return {panel["name"]: panel for panel in layout["workbenches"]["SketcherWorkbench"]}[name]


def test_unified_coincident_is_top_level_next_to_horizontal_vertical(layout):
    panel = _sketcher_panel(layout, "Fusion Sketch Constraints_newPanel")
    top_level = [entry[0] for entry in panel["commands"]]
    assert "Sketcher_ConstrainCoincidentUnified" in top_level
    hor_ver = top_level.index("Sketcher_ConstrainHorVer")
    coincident = top_level.index("Sketcher_ConstrainCoincidentUnified")
    assert coincident - hor_ver == 1, "Coincident should sit right after Horizontal / Vertical"


def test_point_on_object_is_top_level_and_adjacent_to_coincident(layout):
    panel = _sketcher_panel(layout, "Fusion Sketch Constraints_newPanel")
    top_level = [entry[0] for entry in panel["commands"]]
    assert "Sketcher_ConstrainPointOnObject" in top_level
    coincident = top_level.index("Sketcher_ConstrainCoincidentUnified")
    point_on_object = top_level.index("Sketcher_ConstrainPointOnObject")
    assert point_on_object - coincident == 1


def test_legacy_point_only_coincident_is_no_longer_the_primary_button(layout):
    panel = _sketcher_panel(layout, "Fusion Sketch Constraints_newPanel")
    assert "Sketcher_ConstrainCoincident" not in {entry[0] for entry in panel["commands"]}
    # It stays reachable in the panel menu for point-to-point-only compatibility.
    assert "Sketcher_ConstrainCoincident" in {entry[0] for entry in panel.get("overflow", [])}


def test_mirror_with_constraints_is_the_primary_mirror_action(layout):
    panel = _sketcher_panel(layout, "Fusion Sketch Modify_newPanel")
    top_level = {entry[0]: entry for entry in panel["commands"]}
    assert "FusionMyFreeCAD_MirrorWithConstraints" in top_level
    assert top_level["FusionMyFreeCAD_MirrorWithConstraints"][3] == "Mirror + Constraints"
    assert top_level["FusionMyFreeCAD_MirrorWithConstraints"][4] == "Constraint_Symmetric"


def test_native_mirror_stays_visible_next_to_the_advanced_mirror(layout):
    panel = _sketcher_panel(layout, "Fusion Sketch Modify_newPanel")
    commands = panel["commands"]
    top_level = {entry[0]: entry for entry in commands}
    advanced = next(
        i for i, entry in enumerate(commands) if entry[0] == "FusionMyFreeCAD_MirrorWithConstraints"
    )
    native = next(i for i, entry in enumerate(commands) if entry[0] == "Sketcher_Symmetry")
    assert native == advanced + 1
    assert top_level["Sketcher_Symmetry"][2:5] == [
        "small",
        "Mirror",
        "Sketcher_Symmetry",
    ]
...
def test_coincident_buttons_use_their_distinct_native_icons(layout):
    panel = _sketcher_panel(layout, "Fusion Sketch Constraints_newPanel")
    top_level = {entry[0]: entry for entry in panel["commands"]}
    assert top_level["Sketcher_ConstrainCoincidentUnified"][4] == "Constraint_Coincident"
    assert top_level["Sketcher_ConstrainPointOnObject"][4] == "Constraint_PointOnObject"


def test_new_sketch_command_ids_are_tracked_in_the_manifest(layout, manifest):
    for command in (
        "FusionMyFreeCAD_MirrorWithConstraints",
        "Sketcher_ConstrainCoincidentUnified",
        "Sketcher_ConstrainPointOnObject",
    ):
        assert command in manifest["primaryCommands"]
